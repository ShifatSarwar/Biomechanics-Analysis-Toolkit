...
% May 2023 Updated for addition in Nonlinear Analysis Toolkit by Shifat Sarwar, ssarwar@unomaha.edu  
%% Begin Code
...
for i=1:tau
    ...
    n_m1_ktau = double(1/i*sum(nm1));
    n_m_ktau = double(1/i*sum(nm));
...
Bmr = double(sum(Bm)/(N-m));
Amr = double(sum(Am)/(N-m));
SE = -log(Amr/Bmr);
end
